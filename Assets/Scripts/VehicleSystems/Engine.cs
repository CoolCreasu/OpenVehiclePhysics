...
namespace OVP.VehicleSystems
{
    ...
    {
        private float _engineRPM = 0.0f; // Current engine RPM
        ...
        {
            float maxEffectiveTorque = _torqueCurve.Evaluate(_engineRPM); // Get max effective torque from the torque curve
            float friction = _startFriction + _frictionCoefficient * _engineRPM; // Calculate friction
            ...
            _engineRPM = MathExtensions.RadToRPM(EngineAngularVelocity); // Update engine RPM
        }
    }
}